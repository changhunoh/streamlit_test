...
def should_continue(state: StockRecommendationState) -> str:
    print(f"\n--- 계속 여부 확인 중 ---")
    print(f"현재 피드백: {state['feedback']}, 반복 횟수: {state['iteration_count']}")

    if state["feedback"] == "만족" and state["iteration_count"] >= 1: # 최소 1회 추천 후 만족하면 종료
        print(f"DEBUG: LLM Judge가 만족으로 평가하여 워크플로우를 종료합니다. 최종 반복 횟수: {state['iteration_count']}")
        return "end"

    if state["iteration_count"] >= 3: # 최대 3회 반복
        print(f"DEBUG: 최대 반복 횟수(3회)에 도달하여 워크플로우를 종료합니다. 최종 반복 횟수: {state['iteration_count']}")
        return "end"

    print(f"DEBUG: 개선 필요 또는 반복 횟수 미달로 다음 반복을 진행합니다. 현재 반복 횟수: {state['iteration_count']}")
    return "process_query" # 다음 반복 시 LLM에게 추천을 요청하기 위해 process_query로 연결 (뉴스 데이터가 업데이트된 상태로)
...
workflow.add_edge("llm_judge_verifier", "format_recommendations")
...
# Streamlit 앱에서 임포트할 수 있도록 전역 변수로 노출
# nasdaq_stocks_df는 초기 로드 시 생성되므로 별도 처리 필요 없음
